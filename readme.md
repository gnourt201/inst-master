...
wget -qO- inst.sh|bash -s - -选项名 选项值           -t 目标值
   ...
   * 指定硬盘名:            │ -p sda...            │                         │   调试模式用;
   * 指定密码:　            │ -w mypass...         │    gz,app1,app2,app3... │ * 自定义镜像是dd方式
   * 指定网络栈:            │ -6 1                 │    debian,app1,app2...  │   安装的raw系统硬盘格式经过
   * 指定完成后:            │ -o 1/2/3/4           │                         │   gzip/xz打包后托管的http/https地址;
       需扩展磁盘:          │    1:doexpanddisk    │    ./xxx.gz             │   或者qcow2格式的cloudimage
       不注入网络:          │    2:noinjectnetcfg  │    10000:/dev/sda       │   托管的http/https地址
       保持不重启:          │    3:noreboot        │                         │ * 目标名后加隔号隔开app列表为注入app  
       不预先清除:          │    4:nopreclean      │                         │ * ./xxx.gz是本地dd文件名 
       注入穿透件:          │    3389:ratholesrvip │                         │ * 10000:/dev/sda是导出模式     
   * 调试参数项:            │ -d options           │                         │ 
       调试时穿透:          │    22:ratholesrvip   │                         │ 
       改进度端口:          │    vnc:8000          │                         │   
     　　　　　             └────────────────────────────────────────────────┘                                             
