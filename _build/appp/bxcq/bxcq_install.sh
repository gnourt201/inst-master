...
mkdir -p download
wget --no-check-certificate $mirror/bxcq/server.tar.gz -O download/server.tar.gz
wget --no-check-certificate $mirror/bxcq/html.tar.xz -O download/html.tar.xz

BXCQS_DIR=/root/app/bxcq
BXCQC_DIR=/var/www/bxcq
...
sed -i s/129.150.36.60:81/\${CLIENT}/g ${BXCQC_DIR}/index.js
sed -i s/129.150.36.60:81/\${CLIENT}/g ${BXCQC_DIR}/index1.js
sed -e s/129.150.36.60/\${CLIENT%:*}/g -i ${BXCQC_DIR}/GetServerList.php
echo "配置文件修改成功"
