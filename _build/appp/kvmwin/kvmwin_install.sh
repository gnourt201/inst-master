...
wget --no-check-certificate -qO- $mirror/kvmwin/win10x64-ltsc.xml | dos2unix > download/10.xml
wget --no-check-certificate $mirror/kvmwin/virtio-win-1.9.45.tar.xz -O download/drivers.txz
...
wget --no-check-certificate $mirror/kvmwin/windows_secure.rom -O download/windows_secure.rom
wget --no-check-certificate $mirror/kvmwin/windows_secure.vars -O download/windows_secure.vars
...
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
    ...
    wimlib-imagex update tmpinstall/extracted/sources/boot.wim 2 --command "add tmpinstall/10.xml /autounattend.xml" --command "add tmpinstall/10.xml /autounattend.dat"
    # updateimage end
...
-udf -boot-info-table -eltorito-alt-boot -eltorito-boot efi/microsoft/boot/efisys_noprompt.bin -no-emul-boot -allow-limited-size -quiet tmpinstall/extracted
    ...
    qemu-img create -f qcow2 mywin/data.img 50G
fi
EOL
