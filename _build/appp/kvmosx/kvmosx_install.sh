...
wget --no-check-certificate $mirror/kvmosx/boot.img.gz -O download/boot.img.gz
# from https://raw.githubusercontent.com/kholia/OSX-KVM
wget --no-check-certificate $mirror/kvmosx/OVMF_CODE.fd -O download/OVMF_CODE.fd
wget --no-check-certificate $mirror/kvmosx/OVMF_VARS.fd -O download/OVMF_VARS.fd
wget --no-check-certificate $mirror/kvmosx/fetch-macOS-v2.py -O download/fetch.py
...
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
    ...
    (cd myosx; python3 download/fetch.py --action download --board-id Mac-2BD1B31983FE1663)
    gunzip -k -c download/boot.img.gz > myosx/boot.img
    cp download/OVMF_CODE.fd download/OVMF_VARS.fd myosx
    echo "creating osxhd ..."
    qemu-img create -f qcow2 myosx/BigSur-HD.qcow2 50G
...
args=(
 ...
 -drive file=./myosx/com.apple.recovery.boot/BaseSystem.dmg,id=InstallMedia,format=dmg,cache=unsafe,readonly=on,if=none
