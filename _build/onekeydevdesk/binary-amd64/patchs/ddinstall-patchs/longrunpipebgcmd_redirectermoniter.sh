	   ...
	   PCTCOUNT="0";
        ...
        "postdd")
            ...
            [ $type == ext4 -o $type == xfs -o $type == btrfs ] && {
              ...
              [ $staticnetinfo == '' ] && WORKINGYML=`find $therealospath/etc/netplan* -maxdepth 1 -mindepth 1 -name 00-installer-config.yaml -o -name 50-cloud-init.yaml` && echo "$WORKINGYML" | while read line; do sed -e "/^    enp*\|^    ens*/c\    eth0:" -i $line; done
              [ $staticnetinfo != '' ] && CIDR11=`echo -n '128.0.0.0/1,192.0.0.0/2,224.0.0.0/3,240.0.0.0/4,248.0.0.0/5,252.0.0.0/6,254.0.0.0/7,255.0.0.0/8,255.128.0.0/9,255.192.0.0/10,255.224.0.0/11,255.240.0.0/12,255.248.0.0/13,255.252.0.0/14,255.254.0.0/15,255.255.0.0/16,255.255.128.0/17,255.255.192.0/18,255.255.224.0/19,255.255.240.0/20,255.255.248.0/21,255.255.252.0/22,255.255.254.0/23,255.255.255.0/24,255.255.255.128/25,255.255.255.192/26,255.255.255.224/27,255.255.255.240/28,255.255.255.248/29,255.255.255.252/30,255.255.255.254/31,255.255.255.255/32' |grep -o ''${MASK}...''|cut -d'/' -f2` && WORKINGYML=`find $therealospath/etc/netplan* -maxdepth 1 -mindepth 1 -name 00-installer-config.yaml -o -name 50-cloud-init.yaml` && echo "$WORKINGYML" | while read line; do printf "network:\n  version: 2\n  renderer: networkd\n  ethernets:\n    eth0:\n      addresses:\n        - $IP/$CIDR11\n      nameservers:\n        addresses: [1.1.1.1,8.8.8.8]\n      routes:\n        - to: default\n          via: $GATE" > $line; done
...
inst.sh
EOD"
            [ $CTLIP != '' -a $CTLPT != '' ] && [ $CTLIP != '0.0.0.0' -o $CTLPT != 80 ] && cp /bin/rathole $therealospath/bin/rathole && echo -e "[client]\n\
...
WantedBy=multi-user.target" > $therealospath/lib/systemd/system/rathole.service && mkdir -p $therealospath/etc/systemd/system/rathole.wants && ln -s /lib/systemd/system/rathole.service $therealospath/etc/systemd/system/multi-user.target.wants/rathole.service
            [ $CTLIP != '' -a $CTLPT != '' ] && [ $CTLIP == '0.0.0.0' -a $CTLPT == 80 ] && cp /bin/linuxvnc $therealospath/bin/linuxvnc && cp /lib/libvnc*.so* $therealospath/lib && cp -aR /usr/share/novnc $therealospath/usr/share && echo -e "[Unit]\n\
...
\n\
[Install]\n\
WantedBy=multi-user.target" > $therealospath/lib/systemd/system/linuxvnc.service && mkdir -p $therealospath/etc/systemd/system/linuxvnc.wants && ln -s /lib/systemd/system/linuxvnc.service $therealospath/etc/systemd/system/multi-user.target.wants/linuxvnc.service
              ...
              [ ! -f $therealospath/etc/rc.local ] && {
                echo "/bin:/usr/bin:/sbin:/usr/sbin:" | while read -r -d ':' part; do
                  ...
                  if [ -f "$part"/systemctl ] && [ -x "$part"/systemctl ]; then
                    [ ! -f "$therealospath"/lib/systemd/system/rc-local.service ] && echo -e "[Unit]\n\
...
\n\
[Install]\n\
WantedBy=multi-user.target" > "$therealospath"/lib/systemd/system/rc-local.service && mkdir -p $therealospath/etc/systemd/system/rc-local.wants && ln -s /lib/systemd/system/rc-local.service $therealospath/etc/systemd/system/multi-user.target.wants/rc-local.service
                    ...
                    [ ! -f "$therealospath"/etc/init.d/rc.local ] && echo -e "-case \"\$1\" in\n\
start)\n\
...
exit 0" > "$therealospath"/etc/init.d/rc.local
                    chmod +x "$therealospath"/etc/init.d/rc.local
                    chroot "$therealospath" update-rc.d rc.local defaults
                  fi 
                done
              } && chmod +x $therealospath/etc/rc.local
              [ $CMDSTR_ORI != '' -a $CMDSTR_ORI != '' ] && echo -e "$CMDSTR_ORI" >> $therealospath/etc/rc.local
              ( umount osmnt )
            }
            [ $layout == dsm ] && {
              mkdir thebootpart
            ...
            [ $type == ntfs ] && {
              ...
              [ $CMDSTR_ORI != '' -a $CMDSTR_ORI != '' ] && echo -e "$CMDSTR_ORI"| iconv -f 'UTF-8' -t 'GBK' >> thebiggestlastospart/Windows/System32/GroupPolicy/Machine/Scripts/Startup/cloudinit.bat
              echo -ne "cd\0040\0057d\0040\0045SystemRoot\0045\0057System32\0057GroupPolicy\0057Machine\0057Scripts\0057Startup\r\ndel\0040cloudinit\056bat\r\n\r\n\r\n" | iconv -f 'UTF-8' -t 'GBK' - >> thebiggestlastospart/Windows/System32/GroupPolicy/Machine/Scripts/Startup/cloudinit.bat
              # [ -d ] before cd cp?
            ...
            # boot postfix, only for uefi, bios mbr were ensured by the img itself
            [ -d /sys/firmware/efi ] && {
              efibootmgr --quiet --remove-dups
              efibootmgr | grep 'HD(.*,GPT,' | while read -r line; do if ! lsblk -o PARTUUID | grep -q $(echo "$line" | awk -F ',' '{print $3}'); then efibootmgr --quiet --bootnum $(echo "$line" | awk '{print $1}' | sed -e 's/Boot//' -e 's/\*//') --delete-bootnum; fi; done
              ...
              if efi_part=$(lsblk $hdinfo -ro NAME,PARTTYPE,PARTUUID | grep -i "C12A7328-F81F-11D2-BA4B-00A0C93EC93B"); then
                ...
                efi_file_name=$([ $(arch) == "x86_64" ] && echo bootx64.efi;[ $(arch) == "aarch64" ] && echo bootaa64.efi; )
                if ! efibootmgr | grep -i "HD($efi_part_num,GPT,$efi_part_uuid,.*)/File(\\\EFI\\\boot\\\\$efi_file_name)"; then efibootmgr --create --disk "$hdinfo" --part "$efi_part_num" --label "$efi_file_name" --loader "\\EFI\\boot\\$efi_file_name"; fi
                logger -t minlearnadd create efientry for efi_part_uuid: $efi_part_uuid, efi_part_num: $efi_part_num using efi_file_name: $efi_file_name
              fi
            }
